...
if [[ -n "${MAPS_KEY}" ]]; then
  RUN_ENV="${RUN_ENV},GOOGLE_MAPS_SERVER_KEY=${MAPS_KEY},GOOGLE_MAPS_API_KEY=${MAPS_KEY}"
fi
...
if [[ -n "${MAPTILER_API_KEY:-}" ]]; then
  RUN_ENV="${RUN_ENV},MAPTILER_API_KEY=${MAPTILER_API_KEY}"
fi
if [[ -n "${NEXT_PUBLIC_MAPTILER_KEY:-}" ]]; then
  RUN_ENV="${RUN_ENV},NEXT_PUBLIC_MAPTILER_KEY=${NEXT_PUBLIC_MAPTILER_KEY}"
fi
if [[ -n "${NEXT_PUBLIC_MAP_STYLE_URL:-}" ]]; then
  RUN_ENV="${RUN_ENV},NEXT_PUBLIC_MAP_STYLE_URL=${NEXT_PUBLIC_MAP_STYLE_URL}"
fi
